...
card.onReady = function () {
  var $card = $('#' + locals.card.id),
    ...
    $computer = $card.find('select.computer'),
  ...
  $card.find('.add').on('click', function () {
    entries.push({
      ...
      computer: parseInt(parseCount($computer), 10),
    ...
    if (entries.length > 0) {
      ...
      for (var i = 0; i < entries.length; i++) {
        html += '<li>' +
          ...
          '<span class="computer">' + entries[i].computer + '</span>' +
  ...
  function asString() {
    ...
    for (var i = 0; i < entries.length; i++) {
      arrString.push(entries[i].product + '|P: ' + entries[i].physical + '|C: ' + entries[i].computer + '|R: ' + entries[i].required);
    }
